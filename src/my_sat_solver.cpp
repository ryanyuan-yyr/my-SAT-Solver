...
int main()
{
    ...
    vector<TestcaseSpec> testcases{
        ...
        {"uuf100-430", "uuf100-0", 1, 1000}};
    size_t testcase_num = 2;
    for (size_t i = testcases[testcase_num].index_start; i <= testcases[testcase_num].index_end; i++)
    {
        ...
        claim(("unsatisfy", !sat_solver.solve()));
        auto stop = high_resolution_clock::now();
        auto duration = duration_cast<std::chrono::microseconds>(stop - start);
        cout << duration.count() << ' ' << sat_solver.get_statistics().decisionNum << ' ' << sat_solver.get_statistics().backjumpNum << endl;
